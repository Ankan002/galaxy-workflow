		...
		return (
			<SidebarContext.Provider value={value}>
				<div
					ref={ref}
					className={cn(
						"group/sidebar-wrapper flex h-svh min-h-svh w-full overflow-auto",
						className,
					)}
...
>(
	(
		...
		const { state, isMobile, open, openMobile, setOpenMobile } =
			useSidebar();
		...
		const isOffcanvasClosed =
			!isMobile && collapsible === "offcanvas" && !open;
		return (
			<SidebarInner
				ref={ref}
				className={cn(
					...
					className,
					isOffcanvasClosed &&
						"md:w-0 md:min-w-0 md:overflow-hidden md:border-0 md:opacity-0 md:p-0",
				)}
				side={side}
				variant={variant}
...
>(
	(
		...
		const { ripples, handleClick, removeRipple, setRef } = useRipple(
			ripple ?? false,
		);
